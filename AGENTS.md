...
- 初始资金默认 10000.00 元。
- 每日最多 3 次决策，买入、卖出、转换、撤回订单、不操作均计数；单纯净值更新、确认、到账、盈亏快照不计数。
...
- `submitted` 状态的未确认订单可以在确认前撤回；撤回买入会生成 `refund` 现金流水，订单和原关联决策标记为 `cancelled`，原决策次数不回退。撤回本身作为交易决策时使用 `cancel_order`，默认计入每日次数。
